...
compo_table =  table([],[],[],[],[],[], ...
    'VariableNames', {ele_A, ele_B, ele_C, 'Gmin_FCC', 'eq_FCC', 'Gmin_eq_FCC'});

count_fcc_fractions = nan(size(group,1),3);
count_ABCDE         = char(nan(size(group,1), length([char(group(1,1)), '-',char(group(1,2)), '-', char(group(1,3))])));
...
for permu_i = 1: size(group,1)
    ...
    print(gcf,[char(group(permu_i,1)), '-',char(group(permu_i,2)), '-', char(group(permu_i,3)),'_Sputter_CompoContour','.pdf'], '-dpdf')
    ...
    print(gcf, [char(group(permu_i,1)), '-', char(group(permu_i,2)), '-', char(group(permu_i,3)), ...
         '_G comparison_', num2str(tk_Gmin), 'K.pdf'], '-dpdf');
    ...
    % Save plot
    saveas(gcf, [char(group(permu_i,1)), '-', char(group(permu_i,2)), '-', char(group(permu_i,3)), ...
 '_Gmin-FullEquil_Gmin_', num2str(tk_Gmin), 'K', '.pdf'], 'pdf');
    ...
    saveas(gcf, [char(group(permu_i,1)), '-',char(group(permu_i,2)), '-', char(group(permu_i,3)), ...
        '_Gmin-FullEquil_FullEquil', '.pdf'],'pdf');
