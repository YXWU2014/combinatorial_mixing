...
group = perms([{ele_A} {ele_B} {ele_C}]);
% group = [{ele_A} {ele_B} {ele_C}];

compo_table =  table([], [], [], ...
    [], [], [], [], ...
    'VariableNames', {ele_A, ele_B, ele_C, ...
...
for permu_i = 1: size(group,1)
    ...
    [molarV_matgen_A, molarV_matgen_B, molarV_matgen_C] = deal(molarV_matgen{:});
    [YM_matgen_A, YM_matgen_B, YM_matgen_C]             = deal(YM_matgen{:});
    [BM_matgen_A, BM_matgen_B, BM_matgen_C]             = deal(BM_matgen{:});
    [AtomicV_FCC_A, AtomicV_FCC_B, AtomicV_FCC_C]       = deal(AtomicV_FCC{:});
    molarV_ABCDE      = [molarV_matgen_A, molarV_matgen_B, molarV_matgen_C];
    YM_ABCDE          = [YM_matgen_A, YM_matgen_B, YM_matgen_C];
    BM_ABCDE          = [BM_matgen_A, BM_matgen_B, BM_matgen_C];

    YM_eff = NaN(length(PVD_compo),1);
    BM_eff = NaN(length(PVD_compo),1);

    for j = 1: length(PVD_compo)
        PVD_compo_ABCDE_j = PVD_compo(j, 3:5);
    ...
    for j = 1:n_compositions
        ...
        if a~=0
            ...
            delta_prime_misfit_temp = sqrt(c_A_temp * delta_V_A_temp^2 + c_B_temp * delta_V_B_temp^2 + ...
                c_C_temp * delta_V_C_temp^2)/b_FCC_L12_temp^3;
    ...
    compo_data_j = table(PVD_compo(:, 3), PVD_compo(:, 4), PVD_compo(:, 5), ...
        SM_eff, nu_eff, Z_delta_prime_misfit(:, 3), sigma_T, ...
        'VariableNames', {char(group(permu_i,1)), char(group(permu_i,2)), char(group(permu_i,3)), ...
        'ShearModulus_eff', 'PoissonsRatio_eff', 'delta_prime_misfit', 'sigma_SSS'});
    ...
    saveas(gcf, [char(group(permu_i,1)), '-',char(group(permu_i,2)), '-', char(group(permu_i,3)), ...
        '_SolidSolutionStrengthening_', num2str(tk_delta),'K', '.pdf'],'pdf');
