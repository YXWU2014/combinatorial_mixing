...
for permu_i = 1: size(group,1)
    ...
    text(PVD_compo(:,1), PVD_compo(:,2), labels, 'horizontal','center', 'vertical','middle', 'color', [0.5 0.5 0.5])
    
    saveas(gcf, [char(group(permu_i,1)), '-',char(group(permu_i,2)), '-', char(group(permu_i,3)), ...
        '-', char(group(permu_i,4)), '-', char(group(permu_i,5)),'_SolidSolutionStrengthening_', num2str(tk_delta),'K', '.png'],'png');
